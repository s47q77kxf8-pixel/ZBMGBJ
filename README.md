...
- 🎯 **角色档案资料扩展**：新增中/外文名、全名/别名、中文/外文 IP，基础资料固定项（性别/年龄/生日/身高/体重/血型/星座/声优/阵营/身份）、自定义键值资料、语录台词
- 🎯 **角色弹窗二合一**：点击角色名弹出完整角色卡片，可在卡片内一键切换「编辑资料」补充信息；空档案直接进编辑
...
- 🎯 **计算页角色选择器**：「+」打开档案多选，搜索支持中/外文名与 IP；输入新角色失焦自动入档，无匹配可一键新建；下拉默认按 IP 排序
- 🔧 **修复**：角色弹窗嵌在设置子页面内导致排单页点击无法弹出（已移至顶层）
- 🔧 **其他**：移除角色「备注」字段（存量数据保留在存储中）

### 2026-09-10

- 🎯 **单主管理双口径统计**：卡片统计行显示「已结单 N/M · ¥X/¥Y」，分已结单（settlement.type==='normal'）和所有订单（排除撤单与占位单）两组数据；编辑弹窗预览同步展示
- 🎯 **优惠口径选择器**：统一规则和个人优惠自动档位均可独立选择优惠口径（已结单 / 所有订单），两套档位在「合并取最优惠」模式下各自用自己的口径判定命中
- 🎯 **分段选择器 Switch 风格**：统一规则和个人优惠的分段选择器改为带滑动指示器的 Switch 风格，激活态带主题色圆角块 + 缓动动画，更直观
- 🎯 **个人优惠自动档位**：编辑弹窗自动档位新增独立的优惠口径选择器，保存到 `customer.discount.auto.statsBasis`
- 🧠 **计算页自动建档**：在计算页输入新的单主 ID（历史不存在）并失焦时，自动创建单主档案（含接单平台和联系方式）
- 🔧 **默认档位自愈**：首次加载时若检测到统一规则档位全为残留值（阈值 1/3/500），自动替换为推荐合理档位（¥1000/97折、¥3000/95折、5单/97折），并打标避免覆盖后续自定义设置
- 🔧 **修复编辑弹窗折扣功能**：修复客户编辑弹窗折扣输入框缺失 ID 导致保存时报错的问题；手动折扣改为滑块（0.5–1，步进 0.01）
- 🎨 **UI 优化**：分段选择器从两行合并为一行（口径 [已结单|所有] 维度 [金额|单数]），按钮文案缩短、字体减重、圆角放大、加过渡动画；统一规则默认启用自动档位维度切换隔离，防止金额档/单数档互相覆盖
- 🎨 **圆点三态**：单主卡片圆点 — 灰色空心（未达标任何优惠）、蓝色实心（命中统一优惠）、橙色实心（个人优惠）
- 🔒 **接单物料入口隐藏**：接单物料快捷入口默认隐藏，仅在 URL 带 `dev=1` 参数时显示（访问 `index.html?dev=1#settings` 可看到）
