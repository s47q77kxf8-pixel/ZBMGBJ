...
- 📁 **项目结构**：README 项目结构说明更新，纳入预览页与方案文档（制品排单方案、彩条颜色、按钮/导航/小票/彩条预览）
- 📖 **核心文件**：index.html、script.js、style.css 与文档保持同步说明
- 💰 **结算弹窗（结单）**：结单时「本次收款」默认本次应收（可修改）；去除实付金额/差额提示行；显示新平台费、实际单主支付（合计实收 + 新平台费）；保存的金额为本次收款，合计实收 = 已收定金 + 本次收款
- 🎨 **结算优惠布局**：优惠原因（如延期补偿）与「其他原因」均为单行展示：勾选/名称、减去·折扣切换、数值输入框同一行
