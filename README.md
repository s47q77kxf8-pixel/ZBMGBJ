...
- **计算页**：单主信息、用途/加急/折扣/平台费等系数、制品与赠品、工艺与同模，一键计算总价
- **报价页**：小票样式报价单、小票自定义（头图/标题/订单说明/尾图）、保存为图片、历史记录
...
├── README.md           # 本说明
├── 需求.txt            # 产品需求与系数说明
├── 小票制品展示规范.md  # 小票上制品/赠品/工艺的展示规则
└── 小票边框线说明.md   # 小票中各分割线（实线/虚线）的 CSS 位置与用途
```
...
1. 数据与历史均存于浏览器本地，更换设备或清空数据会丢失，重要报价建议及时「保存图片」或另行备份。
2. 打印或导出时，可调整边距以使小票完整显示。
3. 系数、工艺、其他费用等以设置页中当前配置为准，计算页与历史记录会按保存时的设置解释与展示。
